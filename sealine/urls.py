"""
URL configuration for sealine project.

The `urlpatterns` list routes URLs to views. For more information please see:
    https://docs.djangoproject.com/en/5.0/topics/http/urls/
Examples:
Function views
    1. Add an import:  from my_app import views
    2. Add a URL to urlpatterns:  path('', views.home, name='home')
Class-based views
    1. Add an import:  from other_app.views import Home
    2. Add a URL to urlpatterns:  path('', Home.as_view(), name='home')
Including another URLconf
    1. Import the include() function: from django.urls import include, path
    2. Add a URL to urlpatterns:  path('blog/', include('blog.urls'))
"""
from django.contrib import admin
from django.views.static import serve
from django.urls import path, re_path, include

from sealine.settings.base import MEDIA_ROOT
from apps.main.views import MainView


urlpatterns = [
    path('admin/', admin.site.urls),
    path('', MainView.as_view(), name="Main"),
    path('users/', include('apps.users.urls')),
    path('promociones/', include('apps.promotions.urls')),
    path('noticias/', include('apps.news.urls')),
    path('servicios/', include('apps.services.urls')),
    path('tours/', include('apps.tours.urls')),
    path('sobre_nosotros/', include('apps.team.urls')),
    path('testimonios/', include('apps.testimonials.urls')),
    path('tickets/', include('apps.ticket.urls')),
]

urlpatterns += [
    re_path(r'^media/(?P<path>.*)$', serve, {
        'document_root': MEDIA_ROOT,
    })
]